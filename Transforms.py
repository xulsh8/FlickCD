...
class ColorAdjust(object):
    def __init__(self, color_jitter):
        self.cj = transforms.ColorJitter(color_jitter, color_jitter, color_jitter, 0.1)

    def __call__(self, img, label):
        _,H,W = img.shape
        connected_img = torch.empty(3, H, 2*W)
        connected_img[:, :, 0:W] = img[0:3, :, :]
        connected_img[:, :, W:] = img[3:6, :, :]
        process_img = self.cj(connected_img)
        pre_img = process_img[:, :, 0:W]
        post_img = process_img[:, :, W:]
        img = torch.cat((pre_img, post_img), dim=0)
        return [img, label]
...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        label_tensor = torch.LongTensor(np.array(label, dtype=np.int_))
...
class RandomAddShadow(object):
    def __init__(self, num_rectangle=1):
        self.num = num_rectangle
    ...
    def __call__(self, image, label):
        ...
        if random.random() < 0.5:
            for _ in range(self.num):
                ...
                if random.random() < 0.5:
                    image[y1:y2, x1:x2, 0:3] = (image[y1:y2, x1:x2, 0:3] * alpha)
                else:
                    image[y1:y2, x1:x2, 3:6] = (image[y1:y2, x1:x2, 3:6] * alpha)
        return [image, label]
